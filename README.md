...
- `data.py`: Contains a typed version of `data.json5` so I don't have to deal with a dictionary.

# Dependencies

To get this script running, you need the following modules:
- `json5`: Loading and dumping JSON5 files.
- `pydub`: A high-level interface for interacting with music data.
- A ffmpeg or libav binary
- `audioop`: Required for `pydub`. Used to be in the standard library until Python 3.13, future versions require the `audioop-lts` Python library.
...
#### Why don't you use TOML for `data.json5` and `_debug_paths.json5`?
...
About three weeks, mostly working on-and-off, and adding random features.
